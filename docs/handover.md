...
1. **Resend**: domain verified and env vars set in Vercel (2026-09-18). Remaining: send one real form submission and confirm delivery and reply-to; add the key to `.env.local` only if you want to test locally.
...
5. **Deploy**: Vercel project `skralli` under Kode Solutions. Production (2026-09-18, commit 5e02bef): https://skralli.vercel.app — public so Hlynur can view it without a Vercel login, and no-indexed via an `X-Robots-Tag` header on every vercel.app host. Latest preview (login-protected): https://skralli-hcffsfoi2-kode-solutions-44807b0f.vercel.app . Env vars are set in Vercel: `RESEND_API_KEY` and `EMAIL_FROM` for Production and Preview, `CONTACT_TO` for Preview only (production enquiries default to skralli@skralli.is). Still to do: one real test enquiry; connect the GitHub repo for automatic PR previews; merge the PR; attach skralli.is and switch DNS (separate step, nothing is attached today).
...
- 2026-09-19 Image loading: first visits after a deploy were slow because Vercel resizes images on demand, a new deployment starts with an empty image cache, and 45 sources were up to 24 megapixels. Sources are now capped at 2560 px (`npm run images:shrink`; 128 MB -> 66 MB, originals remain in git history; renders at serving size are visually identical), optimized variants are kept for 31 days with fewer widths, and `npm run images:warm <url>` visits every page at phone, laptop and desktop size so no real visitor meets a cold image. Measured 2026-09-19: the optimized-image cache survives a new deployment when the images are unchanged (a fresh deploy warmed with 0 misses), so the main culprits were the 4 hour expiry on a site with no traffic and sizes nobody had requested yet. Run `npm run images:warm https://skralli.vercel.app` after any deploy that adds or changes images or the image config; it takes about 90 seconds and is harmless otherwise. The home hero video also lost its `poster`, which was pulling a raw 1.5 MB PNG on every load. Still open: the hero video itself is 8.3 MB (1080p, 12.7 s, 5.5 Mbps) and competes with the home page's images for bandwidth; re-encoding it to roughly 2.5 MB is the next step if wanted.
